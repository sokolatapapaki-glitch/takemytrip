...
// whose entries carry only a flat `cost`; Rome (below) ships explicit JSON prices.
...
// -----------------------------------------------------------------------------
// Rome — the 26 activities from takemytrip/data/rome.json.
// -----------------------------------------------------------------------------
// Each entry carries the JSON's real metadata (per-age + family `prices`, nearby
// restaurant/cafe, website, notes, tags). The JSON has NO scheduling/scoring data,
// so the engine fields (program/hours/vibes/priority) are filled two ways: the ~12
// activities that already existed in the hand-authored catalogue borrow that
// nextjs object's tuned values; the ~14 new ones get values synthesized from their
// JSON `category` + `tags` (best-effort, tune later).

// --- takemytrip metadata helpers (build the Rome catalogue literals below) ----
...
// A nearby eatery tied to the activity (from the JSON `restaurant` / `cafe`).
const food = (name: string, description: string, link: string | null) =>
  ({ type: "food" as const, price: null, name, description, link, emoji: null });
const cafe = (name: string, description: string, link: string | null) =>
  ({ type: "cafe" as const, price: null, name, description, link, emoji: null });
// An external link (the JSON has only a URL; the display name is generic).
const site = (url: string, name = "Official site") => ({ url, name });

// The catalogue element shape — mirrors core's `Activity` (this file imports
// nothing, so the type is restated and re-checked structurally on import). It's
// annotated on ROME_ACTIVITIES so each entry's `prices.family` is validated as a
// plain Record<string, number>, rather than TS merging the differently-keyed
// family literals (e.g. "4_tickets" vs "2_adults_2_children") into one type.
type CatalogueRestaurant = {
  type: "food" | "cafe";
  price: number | null;
  name: string;
  description: string;
